...
Patcher 网页支持简体中文和英语，自动识别浏览器语言，并在浏览器允许时
记住手动选择。其他语言回退英语；服务端原始诊断信息保留英文。
切换语言不会重新校验文件或丢失已生成的下载链接。

主页同时显示当前 Server、Patch、Patcher 的短提交哈希和源码更新时间（UTC）。
这些信息对应实际部署配置和编译后的 worker，不代表 GitHub 最新版已安装。
旧产物缺少日期时显示“未记录”；内置元数据只有完整提交哈希匹配时才使用，
避免 Patch/Server 独立更新后继续显示旧日期。
